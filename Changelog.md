...
### 19. 🩹 修复一些 BUG
- 实际上我已经忘了修复过什么了🤣
- 但我能肯定的是因为修复而又新增了一些我还未发现的 BUG🫠

### 20. ❓ 新增或功能改变
- 如果您发现有些新增或改变没有写在上方的更新内容中提及，那肯定是我又忘了🤣
...
> ---
>
> XDataHub 是我很长时间以来想要的一种选择加载文件的方式，时间长到在我开始使用 ComfyUI 时就想过了（我开始接触使用 ComfyUI 还是 ComfyUI 项目早期）
> - 我一直在等待 ComfyUI 能够原生拥有类似的功能，但是直到今天 ComfyUI 都没有推出过一个可用的合适的方案，只有在侧边栏里不知道什么人会用的 Assets（资产）作为非常简陋的浏览方式，我很不解 Comfy-Org 团队中的前端开发人员究竟是如何思考的
> - 社区中有许多人开发了各种各样的文件加载节点，这些无私的社区成员制作的节点也是一直以来我使用的方式，但是这些节点都有严重问题一直困扰着我，下面这 2 个是最严重的：
>    - 1. 使用缩略图方式可以很好的预览这些文件（比如图片），但是缩略图都显示在节点前端窗口中会降低整个网页视图的性能流畅度（帧数），一两个这样的节点或许还好，可一旦工作流比较庞大或特殊要求需要多个这样的节点（比如图像编辑模型需要多个不同参考图），那么网页视图的流畅度就会大幅降低，帧数甚至只有几到十几帧
>    - 2. 在缩略图节点这种情况下没有办法切换，要么最小化隐藏掉节点窗口，要么拉远视图触发 Level of Detail 让节点什么都不显示，但是很明显这两个方式都会导致节点无法交互，而且切换起来非常麻烦，没有解决问题反而造成了更多的问题
