...
> - 我一直在等待 ComfyUI 能够原生拥有类似的功能，但是直到今天 ComfyUI 官方都没有推出过一个可用的合适的方案，只有在侧边栏里不知道什么人会用的 Assets（资产）作为非常简陋的浏览方式，我很不解 Comfy-Org 官方团队中的前端开发人员究竟是如何考虑的
> - 社区中有许多人开发了各种各样的文件加载节点，这些无私的社区成员制作的节点也是一直以来我使用的方式，但是这些节点都有严重问题一直困扰着我，下面这 2 个是最严重的：
>    - 1. 使用缩略图方式可以很好的预览这些文件（比如图片），但是缩略图都显示在节点前端窗口中会降低整个工作流的网页视图的性能流畅度（帧数），一两个这样的节点或许还好，可一旦工作流比较庞大或特殊要求需要多个这样的节点（比如图像编辑模型需要多个不同参考图），那么工作流的网页视图的流畅度就会大幅降低，帧数甚至只有几到十几帧
