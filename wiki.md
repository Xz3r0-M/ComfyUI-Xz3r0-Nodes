...
```
# 示例：控制参考图是否传递
input_1 = 参考图 A, enable_1 = Enabled
input_2 = 参考图 B, enable_2 = Disabled
recursive_order = "1-2"

output_1 = 参考图 A
output_2 = None
recursive_output = 参考图 A（第一个有效值）
```
</details>
...
- 图片：从 ComfyUI `input`/`output` 以及 自定义文件夹中读取
- 视频：同上
- 音频：同上
- Lora：从 `models/loras` 文件夹中读取
